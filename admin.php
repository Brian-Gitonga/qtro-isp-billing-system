...
if ($id == "login" || substr($url, -1) == "p") {
...
} elseif (!isset($_SESSION["qtro_isp"])) {
  ...
  if ($API->connect($iphost, $userhost, decrypt($passwdhost))){
    ...
    if ($currency == in_array($currency, $cekindo['indo'])) {
      echo "<script>alert('Qtro ISP not connected!".$nl."Silakan periksa kembali IP, User, Password dan port API harus enable.".$nl."Jika menggunakan koneksi VPN, pastikan VPN tersebut terkoneksi.')</script>";
    }else{
      echo "<script>alert('Qtro ISP not connected!".$nl."Please check the IP, User, Password and port API must be enabled.')</script>";
    }
    if($c == "settings"){
...
?>
<script src="js/qtro-isp-ui.<?= $theme; ?>.min.js"></script>
<script src="js/qtro-isp.js?t=<?= str_replace(" ","_",date("Y-m-d H:i:s")); ?>"></script>
